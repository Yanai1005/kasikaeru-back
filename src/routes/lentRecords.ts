...
lentRecords.post('/', async (c) => {
    try {
        ...
        if (!lent_id || !object_id || !discord_id) {
            return c.json({ error: 'lent_id, object_id, and discord_id are required' }, 400);
        }
    ...
    `).bind(object_id).all();
      ...
      VALUES (?, ?, ?, true)
    `).bind(lent_id, object_id, discord_id).run();

        return c.json({
            message: 'Item lent successfully',
            id: result.meta.last_row_id,
            lent_record: { id: result.meta.last_row_id, lent_id, object_id, discord_id, lent_state: true }
        }, 201);
    } catch (error) {
...
lentRecords.put('/:id/return', async (c) => {
    ...
    `).bind(id).run();
        ...
        if (result.meta.changes === 0) {
            return c.json({ error: 'Lent record not found or already returned' }, 404);
        }
